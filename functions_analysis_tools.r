...
ks_test_manual <- function(X_toFit,cdf1,cdf2,n=length(X_toFit)){
    # performs manual ks test for goodness of fit
    # the implemented ks test of R can only be used to test wether distrs are different
...
fit_plot_reference <- function(x,y,sea,q,state){
    # empty plot with season name, region number etc...
...
fit_plot_combi <- function(distrs,fitstuff,fit_style,sea,q,state,xUntil=60){
    ...
    if (!is.na(fitstuff[15])){
        abline(v=fitstuff[15],col=rgb(0.4,0.4,0.4,1))
