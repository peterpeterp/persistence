...
master_analyse_markov <- function(yearPeriod,trendID,states,transition_names){
    ...
    for (sea in 1:length(seasons)){
        ...
        tmp=global_analysis(per=per,filename_neu=paste("../data/",trendID,"/",states,"_states/markov/",yearPeriod[1],"-",yearPeriod[2],"/",trendID,"_mar",states,"s_trend_",seasons[sea],".nc",sep=""),
            season=seasons[sea],transition_names=transition_names,yearPeriod)
    }
}
...
master_duration <- function(nday,nyr,trendID,states){
    ...
    calc_global_dur(dat=dat,ind=ind,trash=trash,filename=paste("../data/",trendID,"/",states,"_states/duration/",trendID,"_duration_",states,"s.nc",sep=""),states=c(-1,1))

    nc=open.ncdf(paste("../data/",trendID,"/",states,"_states/duration/",trendID,"_duration_",states,"s.nc",sep=""))
...
master_analyse_duration <- function(yearPeriod,trendID,states){
...
master_regional_average <- function(yearPeriod,region_name,trendID){
