...
options(digits=3)
station_plot <- function(dat,trend,per,dur,q,start,stop,season,filename,season_name){
    pdf(file=filename)
    par(mfrow=c(2,1))
    par(mar=c(2,2, 2, 0.5)) 
    ...
    legend("topleft", pch = c(NA,4,NA,NA,NA),lty=c(NA,NA,1,1,1), col = c(NA,"red", "green","yellow","black"), 
            legend = c(season_name,"warm days in a row","markov cold persistence",
    ...
    legend("topleft", pch = c(NA,5,NA,NA,NA),lty=c(NA,NA,1,1,1), col = c(NA,"blue", "green","violet","black"), 
            legend = c(season_name,"cold days in a row","markov cold persistence",
    ...
    if (1==1){
        ...
        plot(hist1$mids,hist1$density,xlim=c(0,endx*2+2),col=rgb(0,1,0,1/2),pch=15,ylim=c(0,max(c(hist1$density,hist2$density))),main=paste("warm periods in",season_name),xlab="duration of period")
        ...
        plot(hist1$mids,hist1$density,xlim=c(0,endx*2+2),col=rgb(0,1,0,1/2),pch=15,ylim=c(0,max(c(hist1$density,hist2$density))),main=paste("cold periods in",season_name),xlab="duration of period")
        points(hist2$mids,hist2$density,col=rgb(1,0,0,1/2),pch=15)
...
for (nday in ndays){
    for (nyr in nyrs){
        for (qq in stations){
            ...
            dur=duration_load(sprintf("../data/%s_%s/%s_%s_duration.nc",nday,nyr,nday,nyr))
            ...
            station_plot(dat=dat,trend=trend,per=per,dur=dur,
                q=qq,start=365*(3)+1,stop=365*(58),season=c(1,365),
                filename=sprintf("../plots/station/%s_%s_year->%s.pdf",nday,nyr,qq),season_name="year")
        }
    }
}
